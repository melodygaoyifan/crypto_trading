...
| **Exit DRL (Discrete SAC)** | EXIT_ONLY (all 3 assets) | PARTIAL_EXIT-only authority; kill switch auto-demotes on 5-loss / DD / drift. |
...
| **Exit-SAC** | `models/exit_drl_v2/` + `core/tick_exit_triggers.py` | EXIT_ONLY (all 3 assets) | Discrete SAC; predicts PARTIAL_EXIT only. Auto-demoted to SHADOW per-asset on consecutive losses / DD / hold-ratio drift / EXIT_ALL spam. |
