...
from infra.failure_policy import classify_external_failure, FailureClass
...
class LLMSentimentAgent:
    ...
    def _classify_haiku_error(err: Exception) -> tuple:
        ...
        # [P345] Delegates to the ONE home for this question. Was hand-rolled
        # here (P329c) alongside a private copy of the date parser — the
        # second implementation of a rule that ~12 sites each re-derive, and
        # that five recorded incidents each got wrong differently. The policy
        # also matches the quota on the MESSAGE before the status, which is
        # what P329c had to discover: a dated cap arrives on 400 and on 429.
        _pol = classify_external_failure(status=status_code, message=msg)
        if _pol.failure_class is FailureClass.QUOTA_EXHAUSTED:
            return (status_code or 400), True, (
                f"usage_limit_{status_code or 400}"
                f"_cooldown={_pol.retry_after_sec:.0f}")
