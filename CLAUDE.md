...
- **Still dead (honest follow-ups, NOT fixed here):** `funding_extreme` needs `funding_sustained_hours`; `vpin_spike` needs computed VPIN threaded (+ `vpin_source=="computed"`); `ml_factor` needs 3 autoencoders trained (`training/factor_extraction/autoencoder_factor.py`); `cascade` is a rare-event strategy ($100M+ liquidations) so low count is partly by design + needs `price_change_4h_pct`.
