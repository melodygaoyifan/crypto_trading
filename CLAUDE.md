...
- **Also caught by the suite: the P253b parquet rebuild had silently DROPPED the 13 fv2_* columns** — `rebuild_pipeline.py` regenerates its own column set and knows nothing about `build_flow_features.py`'s extras, so the P252b parity test went red on the rebuilt files. Restored by re-running `build_flow_features.py` (13 fv2 columns, 100% coverage, all 3 assets; the causal funding column verified untouched, lag-corr still 1.000). **Standing rule: a parquet rebuild is TWO steps — `rebuild_pipeline.py` then `build_flow_features.py` — until the fv2 build is folded into the pipeline.**
