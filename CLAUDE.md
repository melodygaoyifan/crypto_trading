...
- **STILL OPEN (operator-only, cannot be fixed from the repo):** the four deploy secrets must be added at `Settings → Secrets and variables → Actions`. Until then `auto-deploy` stays red and **every deploy must be run manually** with `bash scripts/hetzner_deploy.sh hmats`. Note the deploy script's Step 0 runs `ci_check_invariants.py` via bare `python`; if that interpreter lacks mypy the type gate silently SKIPs (P175) — prefer an interpreter that has it.
