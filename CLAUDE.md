...
- **Why not also touch `_crisis_position_reduction`:** that path (CORR-0 at `main.py:14345`) has the same bug shape — MARKET reduce/exit without cancelling stop first. Not fixed in this commit per scope discipline; runtime evidence shows the FastRiskTick cascade was the live operator pain. CORR-0 only fires during correlation crises and reduces by 25-75% (not full flatten), so balance-lock conflict is much less likely. Filed as P110-followup.
