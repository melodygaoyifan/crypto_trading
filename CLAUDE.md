...
| **Exit DRL (Discrete SAC)** | TRAINING ONLY (not yet wired) | Third DRL alongside the TQC direction DRL (P10). Authority cap = EXIT_ONLY; never decides direction. v1 = 4 actions {HOLD, PARTIAL_EXIT, RELEASE_RUNNER, EXIT_ALL}. Training scripts in `training/exit_drl/`; oracle-labeled trajectories from `training/training_data/drl_training/{ASSET}_4H_full.parquet` via `generate_expert_trajectories.py`; Discrete SAC + CQL via `train_exit_sac.py`; checkpoints land in `models/exit_drl/{ASSET}/exit_sac_best.pt`. **Stage 3 (shadow wiring) deliberately not done yet** — sit at SHADOW for 30 days + ≥30 real exit events, run through existing `DRLPromotionGate` before EXIT_ONLY promotion. |
...
- **System 3 — Exit DRL (Discrete SAC)** (`training/exit_drl/`, NEW 2026-04-24): exit-timing-only discrete SAC over {HOLD, PARTIAL_EXIT, RELEASE_RUNNER, EXIT_ALL}. Trained from oracle-labeled trajectories on the same `{ASSET}_4H_full.parquet` as TQC. Authority cap = EXIT_ONLY. Currently TRAINING ONLY — Stage 3 (shadow wiring into `execution/exit_alpha.py`) deliberately deferred per spec until 30 days of shadow + ≥30 real exit events validate it through `DRLPromotionGate`.
...
- **What lives where:**
  - Trajectory generator: `training/exit_drl/generate_expert_trajectories.py` (oracle uses 48-bar lookahead).
  ...
  - Tests: `tests/test_exit_drl_trajectory_gen.py` + `tests/test_exit_drl_sac.py` (13 tests, all passing).
  - **NO runtime wiring yet.** When Stage 3 lands, the agent goes in `agents/exit_drl_agent.py` (separate file; do **not** repurpose `agents/drl_agent.py` — keeping System 2 vs System 3 in separate modules is intentional).
- **Mitigation:** When `startup_drl_truth.py` is extended for a fourth DRL family, add a System-N row here. The current `DRLPromotionGate` controls only System 1; Exit-SAC needs its own authority gate or a multi-DRL extension to the existing gate. Plan that work explicitly — sharing a single gate across two DRLs would create cross-coupling (a TQC outage demoting Exit-SAC).
