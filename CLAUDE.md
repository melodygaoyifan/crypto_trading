...
- **Kill switch RESET as part of this incident** (recorded manual reset, confirmation semantics `RESET-20260811`): the activation was arithmetic fiction; leaving it tripped blocks the standing keep-trading decision and trains alert-blindness (P202). Reset performed by state-file surgery on the volume (set `kill_switch_active` false, `risk_state` REDUCED — the same conservative re-entry state `reset_kill_switch()` itself uses) with the engine stopped, then a clean boot on the fixed code, verified: no kill restore, P0 fed combined equity, book unchanged.
