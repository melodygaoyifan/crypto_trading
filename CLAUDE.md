...
- **NOT established:** the restart count itself — `ssh hmats` does not resolve from the operator's laptop and cryptopanic.com 403s WebFetch, so the bill was never tied to a measured request count. Settle it with `docker inspect hmats-engine --format '{{.RestartCount}}'`, `docker logs hmats-engine --since 720h | grep -c '\[CRYPTOPANIC\] Initialized'` (= process starts), and the 429 count. **If restarts turn out to be rare, usage really is ~540/mo and the $400 is not usage at all — check whether the subscription tier was changed.**
