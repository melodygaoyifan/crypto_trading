...
| **Exit DRL (Discrete SAC)** | **SHADOW** | Third DRL alongside the TQC direction DRL (P28). Authority cap = EXIT_ONLY; never decides direction. v1 = 4 actions {HOLD, PARTIAL_EXIT, RELEASE_RUNNER, EXIT_ALL}. Training: `training/exit_drl/`; checkpoints `models/exit_drl/{ASSET}/exit_sac_best.pt` (BTC val_align=0.676, ETH=0.677, SOL=0.707, all 50 epochs @ seed=42). Runtime: `agents/exit_drl_agent.py`, instantiated in `_init_components` after `DRLShadowDiagnostics`, per-tick `.predict()` invoked from the agent_signals tick block when there's an active position — SHADOW mode logs to `data/exit_drl_shadow.jsonl` and never influences `execution/exit_alpha.py` triggers. Promotion to EXIT_ONLY requires 30 days shadow + ≥30 real exit events through a *dedicated* Exit-DRL authority gate (do NOT share the TQC `DRLPromotionGate` — see P28). |
...
- **What lives where:**
  ...
  - Tests: `tests/test_exit_drl_trajectory_gen.py` + `tests/test_exit_drl_sac.py` + `tests/test_exit_drl_agent.py` — 20 tests total, all passing.
- **Promotion path (post-shadow):** to flip `ExitDRLMode.SHADOW` → `EXIT_ONLY` after 30 days + ≥30 real exit events:
  1. Build a per-prediction outcome ledger (paper close vs Exit-SAC's prediction at the time the position was closed).
  2. Compute per-action lift vs the rule-based exit baseline using the same logic as `execution/exit_alpha.py`.
  3. Add a dedicated `ExitDRLPromotionGate` (do NOT extend the TQC `DRLPromotionGate` to cover both — System 1 outage shouldn't demote System 3).
  4. Wire `ExitDRLAgent.predict()` into `execution/exit_alpha.py`'s TRIGGER 4 (`DRL_ACTION`) by exposing a `DRLOutput`-compatible bridge (System 3's prediction stands in for the dormant System 2). Only when authority ≥ EXIT_ONLY.
- **Mitigation:** When `startup_drl_truth.py` is extended for a fourth DRL family, add a System-N row here. Sharing a single gate across two DRLs would create cross-coupling.
